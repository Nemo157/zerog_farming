...
File.rename File.join($temp_path, 'default_assets_gravityless_plants', 'gravityless_plants.modinfo'), File.join($temp_path, 'default_assets_gravityless_plants', 'pak.modinfo')
system File.join($starbound_bin_dir, 'asset_packer'), File.join($temp_path, 'default_assets_gravityless_plants'), File.join($output_path, 'gravityless_plants.modpak')
...
$mods_to_override.each do |mod|
  ...
  #puts File.join($starbound_bin_dir, 'asset_packer'), File.join($temp_path, "#{mod}_gravityless_plants"), File.join($output_path, "#{mod}_gravityless_plants.modpak")
